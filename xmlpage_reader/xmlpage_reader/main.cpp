...
static void sigint_handler(int signo) {
    ...
    int length = snprintf(buf, sizeof(buf),
        "Receive signal %d: %s, exit.stop_signal:%d\n",
...
int main(int argc, char *argv[]) {
    ...
    unsigned int listen_port, recv_file_listen_port,
            reader_thread_num, send_task_thread_num;
    ...
    try {
       ...
       cfg.read_value("IF_SEND_FIFE", is_send_file);
        ...
        if (is_send_file &&
            (!cfg.read_value("XML_READER_ADDRESS", xml_reader_address) ||
            xml_reader_address.size() < 7)) {
       ...
       cfg.read_value("IF_RECV_FIFE", is_recv_file);
        ...
        if (is_recv_file &&
            !cfg.read_value("RECV_FILE_LISTEN_PORT", recv_file_listen_port)) {
       ...
       reader_log_error("[BUF] sock_sendbuf_size:%d, sock_recvbuf_size:%d\n",
        ...
        reader_log_error("IF_SEND_FIFE : %s, IF_RECV_FIFE : %s\n",
                        is_send_file == false ? "FALSE" : "TRUE",
                        is_recv_file == false ? "FALSE" : "TRUE");
        // init XmlItemMgr
       if (g_xmlItemMgr.init() != 0) {
        ...
        if (is_send_file) {
           ...
           reader_log_error("[INIT] send init file_id %d\n",
            ...
            while (xml_send_file->send_file(NULL, SEND_INIT_FILE_ID,
                    g_xmlItemMgr.init_file_id) != 0) {
       ...
       for (int i=0; i < g_send_tasks.size(); i++) g_send_tasks[i]->open(NULL);
        ...
        if (!is_recv_file) {
           ...
           NetworkInputAck acceptor(ACE_Reactor::instance(
                new ACE_Reactor(new ACE_Dev_Poll_Reactor())), 1, false);
           acceptor.setServer(&xmlPage_reader);
           if (acceptor.open(addr) < 0) {
             reader_log_error(
              "XmlPageReader acceptor open::port already in use,port=%d\n",
              listen_port);
             stop_task(xmlPage_reader);
             return -1;
           }
