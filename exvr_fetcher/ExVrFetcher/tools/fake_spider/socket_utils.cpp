...
*/
	...
	fprintf(stderr,"[asyn_connect]connect fd=%d ERROR,errno=%d\n",socket_fd,errno);
	return -1;
}
...
int getHostofUrl(const std::string &_url,int &retBeg,int &retEnd)
{
	...
	if( (pos1==std::string::npos) &&(pos1_1==std::string::npos)) 
	{   
		retBeg=0;
	}
	else if(pos1_1==std::string::npos)
		retBeg=pos1+7;
	else
		retBeg=pos1+8;
	int pos2=_url.find("/",retBeg);
	int pos3=_url.find(":",retBeg);
	if ((pos3!=std::string::npos)&&((pos2==std::string::npos)||(pos3<pos2)))
	{
		pos2 = pos3;
	}   
...
int get_host_port(const std::string &strAddr,std::string &_host,int &_port)
{
	int _beg,_end;
	...
	if(_port<=0) 
	{
		if( strAddr.find("http://")!=std::string::npos)
			_port=80;
		else if(strAddr.find("https://")!=std::string::npos )
			_port=443;
		else
		{   
...
std::string hostnameToIp(std::string name)
{
    ...
    //std::string ip="pic.sogou.com";
	//获取ip失败会返回非0返回值...
    if(gethostbyname_r(name.c_str(),&hostinfo,buf,sizeof(buf),&phost,&ret))  
    {
        fprintf(stderr,"[ERROR][hostnameToIp] get host by name error\n");
        return "";
    }
	//解析ip失败，返回NULL
	if(phost == NULL)
	{
		fprintf(stderr,"[ERROR][hostnameToIp] get host by name parse error\n");
		return "";
	}
    for(int i = 0;phost->h_addr_list[i];i++)  
    {
        //res.assgin(phost->h_addr_list[i]);
        
        //fprintf(stderr,"fmm_test addr is  %d %d %x\n",phost->h_addrtype,i,*(unsigned long*)phost->h_addr_list[i]);
        //找到一台就可以退出...
        if(inet_ntop(phost->h_addrtype,phost->h_addr_list[i], str, sizeof(str)))
        {
            res.assign(str,strlen(str));
            //fprintf(stderr,"fmm_test hostnameToIp ip %s\n",res.c_str());
            break;
        }
        //fprintf(stderr,"fmm_test errorno %d\n",errno);
    }
    return res;
}
