...
{
	return s_iconv(src+'\0',result,"GB18030","utf-16");
}
...
std::string str_add_num(std::string str,int index)
{
    ...
    std::string query;
    memset(buf,0,32);
    snprintf(buf,32,"%s%d",str.c_str(),index);
    query.assign(buf);
    return query;
}
...
int togchar(std::string &strtmp,std::string &dest)
{
    ...
    gchar_t buf[256] = {0};
    if(EncodingConvertor::getInstance()->dbc2gchar(strtmp.c_str(), buf, 256, true) <= 0)
        return -1;
    size_t length = strlen((const char*)buf) / 2;
    for (int p = 0; p < length; p++)
    {
        if (memcmp(buf + p, "　", sizeof(gchar_t)) == 0 ||
                memcmp(buf + p, "，", sizeof(gchar_t)) == 0 ||
                memcmp(buf + p, "。", sizeof(gchar_t)) == 0)
        {
            buf[p] = 0x0920;
        }
