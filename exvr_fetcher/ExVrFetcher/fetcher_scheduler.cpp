...
int main(int argc, char* argv[]) {
    ...
    SS_LOG_UTIL->open();
    u_long priority_mask = SS_LOG_UTIL->process_priority_mask(
        ACE_Log_Msg::PROCESS);
    u_long log_flags = 0;
    ...
    // WEBSEARCH_LOG->msg_ostream(new ofstream(
                    // Config_Manager::instance()->get_log_file().c_str(),
                    // ios::trunc|ios::ate));
    ...
    std::string check_interval_str = section.Value<std::string>(
        "CheckInterval");
    ...
    if (check_interval_str == "" || scan_interval_str == "" ||
        check_interval_vec.size() != scan_interval_vec.size()) {
        SS_DEBUG((LM_ERROR,
                "Configuration CheckInterval and CheckInterval error %s %s\n",
    ...
    if (check_interval_vec.size() != spider_type_set.size()) {
        SS_DEBUG((LM_ERROR,
                "Configuration CheckInterval or SpiderType error %s %s\n",
                check_interval_str.c_str(), spider_type_str.c_str()));
        return -1;
    }

    for (int i = 0; i < spider_type_vec.size(); i++) {
        if (FetcherScheduleScan::init(conf, spider_type_vec[i],
            check_interval_vec[i], scan_interval_vec[i]) < 0) {
            SS_DEBUG((LM_ERROR, "Configuration error\n"));
            return -1;
        }
    }
    //  start fetcher_engine_scan threads
    for (int i = 0; i < fetcher_schedule_scans.size(); i++)
        fetcher_schedule_scans[i]->open(NULL);
    ...
    //  begin to exit...
    for (int i=0; i< fetcher_schedule_scans.size(); i++)
        fetcher_schedule_scans[i]->stopTask();
